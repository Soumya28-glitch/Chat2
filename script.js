...
function lockApp() {
    if (document.getElementById('chat').style.display === 'flex') {
        ...
        if (chatRef) {
            chatRef.off(); // Stop listening to Firebase in the background
            chatRef = null;
        }
        
        const btn = document.getElementById('unlock-btn');
        if (btn) { btn.innerText = "Unlock Room (Ready)"; btn.style.opacity = "1"; }
        document.getElementById('messages').innerHTML = ''; // Wipe screen
    }
}
...
function joinChat() {
    try {
        ...
        const btn = document.getElementById('unlock-btn');
        btn.innerText = "Decrypting Database...";
        btn.style.opacity = "0.7";

        if (typeof CryptoJS === 'undefined' || typeof firebase === 'undefined') {
            return showError("Libraries blocked. Check internet.");
        }
        ...
        // Hash the PIN so the database folder name is a secret too
