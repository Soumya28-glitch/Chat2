...
function lockApp() {
    if (document.getElementById('chat').style.display === 'flex') {
        ...
        if (btn) { btn.innerText = "Unlock Room"; btn.style.opacity = "1"; }
...
function initApp() {
    ...
    if(unlockBtn) {
        unlockBtn.innerText = "Unlock Room";
    ...
    try {
        ...
        const syncTimeout = setTimeout(() => {
            if(document.querySelectorAll('.msg-wrapper').length === 0) {
                addSystemMessage("Sync taking longer than usual. Check internet or PIN.");
            }
        }, 8000);
...
function setupMicLogic() {
    ...
    function startAudio() {
        navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
            ...
            mediaRecorder.start();
        }).catch(err => addSystemMessage("Mic Error: " + err.message));
    }
...
function renderMessage(sender, content, isMine, time, isFile, fileName, fileType, isVoiceNote) {
    ...
    if (isFile) {
        if (isVoiceNote || (fileType && fileType.startsWith('audio/'))) msgDiv.innerHTML = `<audio controls controlsList="nodownload noplaybackrate"><source src="${content}"></audio>`;
        else if (fileType && fileType.startsWith('image/')) msgDiv.innerHTML = `<img src="${content}" style="max-width:100%; border-radius:8px;">`;
